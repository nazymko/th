...
public class SyncMainLogDao extends AbstractDao<Integer, ConnectorSyncMainLogRecord> {
    ...
    public Long getLastPage(String consumer) {
        Record1<Long> stringRecord1 = getDslContext().select(DSL.max(CONNECTOR_SYNC_MAIN_LOG.LATEST_PAGE_ID)).from(CONNECTOR_SYNC_MAIN_LOG).where(CONNECTOR_SYNC_MAIN_LOG.CONSUMER.eq(consumer)).fetchOne();
